import http.server, socketserver, json, os, base64, re, io
...
def F(id,type,sev,page,region,cur,sug,reason):
    return dict(id=id,type=type,severity=sev,page=page,region=region,current=cur,suggested=sug,reason=reason,status="open")
...
def analyse(pdf_bytes):
    ...
    if not findings:
        findings.append(F(nid(),"info","low",1,"Full drawing","","","No automated anomalies detected. Manual review still recommended."))
    return findings

def check(text,pn,nid):
    f=[]; t=text.upper()
    for m in TIGHT.finditer(text):
        f.append(F(nid(),"extreme_tolerance","high",pn,f"Near: {m.group()}",m.group(),"+-0.01 (verify necessity)","Tolerance tighter than +-0.005 is very hard to manufacture and inspect."))
    for m in LOOSE.finditer(text):
        f.append(F(nid(),"loose_tolerance","medium",pn,f"Near: {m.group()}",m.group(),"Review against fit/function","Tolerance of +-5 or greater is unusually loose."))
    MAT=["MATERIAL","MAT:","ALLOY","STEEL","ALUMIN","PLASTIC","BRASS","TITANIUM","COPPER","NYLON"]
    if not any(w in t for w in MAT):
        f.append(F(nid(),"missing_material","high",pn,"Title block","No material found","Add e.g. EN AW-6082-T6","Every drawing must specify material."))
    if not any(w in t for w in ["FINISH","ROUGHNESS","RA ","RZ "]) and "Ra" not in text:
        f.append(F(nid(),"missing_surface_finish","medium",pn,"Title block","No surface finish","Add e.g. Ra 1.6","Surface finish missing."))
    if not any(w in t for w in ["UNLESS OTHERWISE","GENERAL TOL","ISO 2768","ASME Y14","BS 8888"]):
        f.append(F(nid(),"missing_general_tolerance","medium",pn,"General notes","No general tolerance note","UNLESS OTHERWISE SPECIFIED: Linear +-0.1mm Angular +-0.5deg","Untoleranced dimensions cause disputes."))
    if not any(w in t for w in ["REV","REVISION"]):
        f.append(F(nid(),"missing_revision","low",pn,"Title block","No revision","Add e.g. Rev A","All drawings need a revision level."))
    if not any(w in t for w in ["SCALE","1:1","1:2","2:1","1:5","1:10"]):
        f.append(F(nid(),"missing_scale","low",pn,"Title block","No scale","Add e.g. SCALE 1:1","Scale not declared."))
    dims = DIM.findall(text)
    if not dims and len(text.strip())>80:
        f.append(F(nid(),"missing_dimensions","high",pn,"Full page","No dimensions found","Verify all dims present","No dimension values found. Drawing may be incomplete."))
    if dims and not TOL.findall(text):
        f.append(F(nid(),"dims_without_tolerances","medium",pn,"Full page",f"{len(dims)} dims, no tolerances","Add tolerances","Dimensions found but no tolerances."))
    return f
...
def dupes(text,nid):
    ...
    for val,n in Counter(vals).items():
        if n>=4:
            f.append(F(nid(),"duplicate_dimension","medium",1,"Multiple",f"'{val}' x{n}","Verify no conflicts",f"'{val}' repeated {n} times."))
    return f
...
def export(pdf_bytes, approved):
    try:
        ...
        return pdf_bytes
    if not approved: return pdf_bytes
    reader  = pypdf.PdfReader(io.BytesIO(pdf_bytes))
